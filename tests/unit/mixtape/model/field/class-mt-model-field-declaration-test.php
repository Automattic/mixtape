...
class MT_Model_Field_DeclarationTest extends MT_Testing_Model_TestCase {
    function test_exists() {
        $this->assertClassExists( 'MT_Model_Field_Declaration' );
    }
    ...
    function test_construct_throws_if_name_not_provided() {
        new MT_Model_Field_Declaration( array() );
    }
    ...
    function test_construct_throws_if_type_not_provided() {
        new MT_Model_Field_Declaration( array('name' => 'foo') );
    }
    ...
    function test_construct_throws_if_type_not_valid() {
        new MT_Model_Field_Declaration( array('name' => 'foo', 'type' => 'zap') );
    }

    function test_field_declarations() {
        $registry = $this->environment->type();

        $sum_declaration = (new MT_Model_Field_Declaration_Builder())
            ->with_data_store_type(MT_Model_Field_Declaration::FIELD)
            ->named( 'sum' )
        ...
        $first_name_declaration = (new MT_Model_Field_Declaration_Builder())
            ->with_data_store_type(MT_Model_Field_Declaration::FIELD)
            ->named( 'first_name' )
        ...
        $derived_declaration = (new MT_Model_Field_Declaration_Builder())
            ->with_data_store_type(MT_Model_Field_Declaration::DERIVED)
            ->named( 'derived' )
            ->build();
    ...
    function test_get_description_default_to_name() {
        $field =(new MT_Model_Field_Declaration_Builder())
            ->with_data_store_type(MT_Model_Field_Declaration::FIELD)
    ...
    function test_as_item_schema_property_contain_choices_if_set() {
        ...
        $builder->with_data_store_type(MT_Model_Field_Declaration::FIELD)
            ->choices( $choices )
