...
class Mixtape_Model_Field_Declaration_Builder {
	...
	function __construct() {
		$this->args = array(
			'name'               => '',
			'type'               => Mixtape_Model_Field_Declaration::FIELD,
			'type_definition'    => Mixtape_Type::any(),
			'required'           => false,
			'map_from'           => null,
			'before_return'      => null,
			'sanitize'           => null,
	 ...
	 *
	 * @param Mixtape_Interfaces_Type $value_type
	 * @return Mixtape_Model_Field_Declaration_Builder $this
	 * @throws Mixtape_Exception
	 */
	public function typed( $value_type ) {
		if ( ! is_a( $value_type, 'Mixtape_Interfaces_Type' ) ) {
			throw new Mixtape_Exception( get_class( $value_type ) . ' is not a Mixtape_Interfaces_Type' );
		}
	...
	public function derived( $func ) {
		return $this->with_data_store_type( Mixtape_Model_Field_Declaration::DERIVED )->map_from( $func );
	}
}
